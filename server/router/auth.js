...
router.post("/register",async(req,res)=>{
    ...
    try{
        ...
        if(userExist){
            ...
            await user.save()
            res.status(200).json({message:"sucsessfull save the data on mongodb"})
        }   
...
router.post("/login",async(req,res)=>{
    try{
        ...
        if(!userLogin){
            res.status(400).json({error:"Email not found plaaz signup first"})   
        }
        else{
    ...
    if(emailExist){
        ...
        try{
            ...
            const details = {
                ...
                to:email,
                subject:`Your reset password OTP is ${OtpCode}`,
